import { auditInTransaction } from "@backend/lib/audit";
import { prisma } from "@backend/lib/prisma";
import { createDebtPaymentSnapshot } from "@backend/lib/snapshots";

import type { ServiceContext } from "@backend/lib/audit";
import type { Logger } from "@jahonbozor/logger";
import type {
    DebtOrderItem,
    DebtOrdersResponse,
    DebtPaymentResponse,
    DebtPaymentsListResponse,
    DebtSummaryResponse,
} from "@jahonbozor/schemas/src/debts";

interface DebtTotalRow {
    totalDebt: number;
    debtOrdersCount: number;
}

interface PaidTotalRow {
    totalPaid: number;
}

export abstract class DebtsService {
    static async getUserDebtSummary(userId: number, logger: Logger): Promise<DebtSummaryResponse> {
        try {
            const [debtTotals, paidTotals] = await Promise.all([
                prisma.$queryRaw<DebtTotalRow[]>`
                    SELECT
                        COALESCE(SUM(oi.price * oi.quantity), 0)::float8 AS "totalDebt",
                        COUNT(DISTINCT o.id)::integer AS "debtOrdersCount"
                    FROM "Order" o
                    JOIN "OrderItem" oi ON oi."orderId" = o.id
                    WHERE o."userId" = ${userId}
                        AND o."paymentType" = 'DEBT'
                        AND o."status" != 'CANCELLED'
                        AND o."deletedAt" IS NULL
                `,
                prisma.$queryRaw<PaidTotalRow[]>`
                    SELECT COALESCE(SUM(amount), 0)::float8 AS "totalPaid"
                    FROM "DebtPayment"
                    WHERE "userId" = ${userId}
                `,
            ]);

            const totalDebt = debtTotals[0]?.totalDebt ?? 0;
            const debtOrdersCount = debtTotals[0]?.debtOrdersCount ?? 0;
            const totalPaid = paidTotals[0]?.totalPaid ?? 0;
            const balance = totalDebt - totalPaid;

            return {
                success: true,
                data: { totalDebt, totalPaid, balance, debtOrdersCount },
            };
        } catch (error) {
            logger.error("Debts: Error in getUserDebtSummary", { userId, error });
            return { success: false, error };
        }
    }

    static async getDebtOrders(userId: number, logger: Logger): Promise<DebtOrdersResponse> {
        try {
            const orders = await prisma.order.findMany({
                where: {
                    userId,
                    paymentType: "DEBT",
                    status: { not: "CANCELLED" },
                    deletedAt: null,
                },
                include: {
                    items: true,
                    debtPayments: {
                        include: { staff: { select: { id: true, fullname: true } } },
                        orderBy: { paidAt: "desc" },
                    },
                },
                orderBy: { createdAt: "desc" },
            });

            const debtOrders: DebtOrderItem[] = orders.map((order) => {
                const orderTotal = order.items.reduce(
                    (sum, item) => sum + Number(item.price) * item.quantity,
                    0,
                );
                const paidAmount = order.debtPayments.reduce((sum, p) => sum + Number(p.amount), 0);

                return {
                    orderId: order.id,
                    userId: order.userId,
                    orderTotal,
                    paidAmount,
                    remainingAmount: orderTotal - paidAmount,
                    status: order.status,
                    createdAt: order.createdAt,
                    payments: order.debtPayments.map((p) => ({
                        id: p.id,
                        orderId: p.orderId,
                        userId: p.userId,
                        amount: Number(p.amount),
                        paidAt: p.paidAt,
                        staffId: p.staffId,
                        comment: p.comment,
                        staff: p.staff,
                        createdAt: p.createdAt,
                    })),
                };
            });

            return { success: true, data: { orders: debtOrders } };
        } catch (error) {
            logger.error("Debts: Error in getDebtOrders", { userId, error });
            return { success: false, error };
        }
    }

    static async createDebtPayment(
        orderId: number,
        body: { amount: number; comment?: string | null },
        context: ServiceContext,
        logger: Logger,
    ): Promise<DebtPaymentResponse> {
        try {
            const order = await prisma.order.findUnique({
                where: { id: orderId },
            });

            if (!order) {
                logger.warn("Debts: Order not found for payment", { orderId });
                return { success: false, error: "Order not found" };
            }

            if (order.paymentType !== "DEBT") {
                logger.warn("Debts: Order is not DEBT type", {
                    orderId,
                    paymentType: order.paymentType,
                });
                return { success: false, error: "Order is not DEBT type" };
            }

            if (order.deletedAt) {
                logger.warn("Debts: Cannot pay for deleted order", { orderId });
                return { success: false, error: "Cannot pay for deleted order" };
            }

            const userId = order.userId;
            if (!userId) {
                logger.warn("Debts: Order has no associated user", { orderId });
                return { success: false, error: "Order has no associated user" };
            }

            // Calculate remaining amount to prevent overpayment
            const orderWithItems = await prisma.order.findUnique({
                where: { id: orderId },
                include: {
                    items: { select: { price: true, quantity: true } },
                    debtPayments: { select: { amount: true } },
                },
            });

            if (orderWithItems) {
                const orderTotal = orderWithItems.items.reduce(
                    (sum, item) => sum + Number(item.price) * item.quantity,
                    0,
                );
                const totalPaid = orderWithItems.debtPayments.reduce(
                    (sum, p) => sum + Number(p.amount),
                    0,
                );
                const remaining = orderTotal - totalPaid;

                if (body.amount > remaining) {
                    logger.warn("Debts: Payment amount exceeds remaining debt", {
                        orderId,
                        amount: body.amount,
                        remaining,
                    });
                    return {
                        success: false,
                        error: "Payment amount exceeds remaining debt",
                    };
                }
            }

            const [payment] = await prisma.$transaction(async (transaction) => {
                const created = await transaction.debtPayment.create({
                    data: {
                        orderId,
                        userId,
                        amount: body.amount,
                        staffId: context.staffId,
                        comment: body.comment ?? null,
                    },
                    include: {
                        staff: { select: { id: true, fullname: true } },
                    },
                });

                await auditInTransaction(
                    transaction,
                    { requestId: context.requestId, user: context.user, logger },
                    {
                        entityType: "debt_payment",
                        entityId: created.id,
                        action: "DEBT_PAYMENT",
                        newData: createDebtPaymentSnapshot(created),
                    },
                );

                return [created];
            });

            logger.info("Debts: Payment recorded", {
                paymentId: payment.id,
                orderId,
                amount: body.amount,
                staffId: context.staffId,
            });

            return {
                success: true,
                data: {
                    id: payment.id,
                    orderId: payment.orderId,
                    userId: payment.userId,
                    amount: Number(payment.amount),
                    paidAt: payment.paidAt,
                    staffId: payment.staffId,
                    comment: payment.comment,
                    staff: payment.staff,
                    createdAt: payment.createdAt,
                },
            };
        } catch (error) {
            logger.error("Debts: Error in createDebtPayment", { orderId, error });
            return { success: false, error };
        }
    }

    static async getDebtPayments(
        orderId: number,
        logger: Logger,
    ): Promise<DebtPaymentsListResponse> {
        try {
            const payments = await prisma.debtPayment.findMany({
                where: { orderId },
                include: {
                    staff: { select: { id: true, fullname: true } },
                },
                orderBy: { paidAt: "desc" },
            });

            return {
                success: true,
                data: {
                    payments: payments.map((p) => ({
                        id: p.id,
                        orderId: p.orderId,
                        userId: p.userId,
                        amount: Number(p.amount),
                        paidAt: p.paidAt,
                        staffId: p.staffId,
                        comment: p.comment,
                        staff: p.staff,
                        createdAt: p.createdAt,
                    })),
                },
            };
        } catch (error) {
            logger.error("Debts: Error in getDebtPayments", { orderId, error });
            return { success: false, error };
        }
    }
}
