...
export const publicOrders = new Elysia({ prefix: "/orders" })
    ...
    .post(
        "/",
        async ({ body, user, type, set }): Promise<ReturnSchema> => {
            try {
                if (type !== "user") {
                    set.status = 403;
                ...
                const result = await PublicOrdersService.createOrder(body, user.id);
    ...
    .get(
        "/",
        async ({ query, user, type, set }): Promise<ReturnSchema> => {
            try {
                if (type !== "user") {
                    set.status = 403;
                ...
                return await PublicOrdersService.getUserOrders(user.id, query);
            } catch (error) {
    ...
    .get(
        "/:id",
        async ({ params, user, type, set }): Promise<ReturnSchema> => {
            try {
                if (type !== "user") {
                    set.status = 403;
                    return { success: false, error: "Only users can access this endpoint" };
                ...
                const result = await PublicOrdersService.getUserOrder(params.id, user.id);
