...
// Mock JWT signer
const createMockJwt = () => ({
	sign: vi.fn((payload: Record<string, unknown>) => Promise.resolve("mock-jwt-token")),
});
...
describe("PublicUsersService", () => {
	...
	describe("authenticateWithTelegram", () => {
		test("should authenticate new user via Telegram", async () => {
			...
			const saveTokenSpy = vi.spyOn(Auth, "saveRefreshToken").mockResolvedValue(true);
		...
		test("should return failure when saveRefreshToken fails", async () => {
			...
			const saveTokenSpy = vi.spyOn(Auth, "saveRefreshToken").mockResolvedValue(null);
		...
		test("should accept request when optional fields are undefined", async () => {
			...
			const newUser = { ...mockUser, id: 2, fullname: "Jane", username: "987654321", telegramId: "987654321" };
			prismaMock.users.findUnique.mockResolvedValue(null);
			mockTransaction({
				users: { create: () => Promise.resolve(newUser) },
				auditLog: { create: () => Promise.resolve({}) },
			...
			// Act
			const result = await PublicUsersService.authenticateWithTelegram(
				body as unknown as Parameters<typeof PublicUsersService.authenticateWithTelegram>[0],
				mockJwt,
				{ requestId: "test-req" },
				mockLogger,
			);
	...
	describe("updateLanguage", () => {
		...
		test("should update user language successfully", async () => {
			...
			// Act
			const result = await PublicUsersService.updateLanguage(1, "ru", mockContext, mockLogger);
		...
		test("should return failure when user not found", async () => {
			...
			// Act
			const result = await PublicUsersService.updateLanguage(999, "ru", mockContext, mockLogger);
		...
		test("should accept uz language", async () => {
			...
			// Act
			const result = await PublicUsersService.updateLanguage(1, "uz", mockContext, mockLogger);
