...
describe("income.api", () => {
	...
	describe("incomeListQueryOptions", () => {
		...
		test("queryFn should return data on success", async () => {
			...
			const result = await incomeListQueryOptions().queryFn!(
				{} as ListCtx,
			);
		...
		test("queryFn should throw on error", async () => {
			...
			await expect(
				incomeListQueryOptions().queryFn!({} as ListCtx),
			).rejects.toBeDefined();
		});
	...
	describe("createIncomeFn", () => {
		...
		test("should throw on error", async () => {
			...
			await expect(
				createIncomeFn({ productId: 1, quantity: 0 }),
			).rejects.toBeDefined();
		});
		...
		test("should throw when success is false", async () => {
			...
			await expect(
				createIncomeFn({ productId: 1, quantity: 10 }),
			).rejects.toThrow("Request failed");
		});
	});
});
