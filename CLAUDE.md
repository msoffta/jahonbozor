...
5. **Интерактивность** — все фронтенд-приложения должны быть интерактивными и приятными: `PageTransition` на каждой странице, `whileTap` на кнопках, `AnimatePresence` для conditional renders, spring-анимации, правильные типы курсора (pointer/text/not-allowed — задано глобально). См. [docs/frontend.md](docs/frontend.md#animation-motion)
6. **Размер CLAUDE.md** — файл не должен превышать 40,000 символов. При добавлении нового контента, если лимит превышен, выносить детали в `docs/` и оставлять ссылку
